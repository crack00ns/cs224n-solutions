...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        W = tf.get_variable("W", shape=[self.config.n_features, self.config.n_classes], dtype=tf.float32, initializer=tf.zeros_initializer)
        b = tf.get_variable("b", shape=[1, self.config.n_classes], dtype=tf.float32, initializer=tf.zeros_initializer)
