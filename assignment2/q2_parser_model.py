...
class ParserModel(Model):
    ...
    def add_prediction_op(self):
        ...
        weight_init = xavier_weight_init()
        
        if self.config.extension_on:
            self.config.dropout = 0.2
            self.config.n_epochs = 12
            W = weight_init((n_features*embed_size, hidden_size))
            b1 = tf.Variable(tf.zeros((hidden_size,)))

            W_extra = weight_init((hidden_size, hidden_size/2))
            b_extra = tf.Variable(tf.zeros((hidden_size/2,)))

            U = weight_init((hidden_size/2, n_classes))
            b2 = tf.Variable(tf.zeros((n_classes,)))

            h = tf.nn.relu(tf.matmul(x, W)+b1)
            h_drop = tf.nn.dropout(h, keep_prob=1-self.dropout_placeholder)

            h = tf.nn.relu(tf.matmul(h_drop, W_extra)+b_extra)
            h_drop = tf.nn.dropout(h, keep_prob=1-self.dropout_placeholder)
            pred = tf.matmul(h_drop, U) + b2
        else:
            W = weight_init((n_features*embed_size, hidden_size))
            b1 = tf.Variable(tf.zeros((hidden_size,)))
            U = weight_init((hidden_size, n_classes))
            b2 = tf.Variable(tf.zeros((n_classes,)))
