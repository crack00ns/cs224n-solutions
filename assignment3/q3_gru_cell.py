...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_r = tf.get_variable("W_r", (self.input_size, self._state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            U_r = tf.get_variable("U_r", (self._state_size, self._state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            b_r = tf.get_variable("b_r", (self._state_size,), initializer=tf.constant_initializer(0), dtype=np.float32)
            W_z = tf.get_variable("W_z", (self.input_size, self._state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            U_z = tf.get_variable("U_z", (self._state_size, self._state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            b_z = tf.get_variable("b_z", (self._state_size,), initializer=tf.constant_initializer(0), dtype=np.float32)
            W_o = tf.get_variable("W_o", (self.input_size, self._state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            U_o = tf.get_variable("U_o", (self._state_size, self._state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            b_o = tf.get_variable("b_o", (self._state_size,), initializer=tf.constant_initializer(0), dtype=np.float32)
