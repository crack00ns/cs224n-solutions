    ...
    for sentence, labels in data:
        ...
        for index, (word, label) in enumerate(zip(sentence, labels)):
            ...
            for k in range(-window_size, window_size+1):
                if k+index<0:
                    single_window.extend(start)
                elif k+index>=len(sentence):
                    single_window.extend(end)
                else:
                    single_window.extend(sentence[k+index])
            windowed_data.append((single_window, label))
		# ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        n_window_features, embed_size, hidden_size, n_classes = (self.config.n_window_features, self.config.embed_size,
                                                                 self.config.hidden_size, self.config.n_classes)
        W = tf.get_variable("W", shape=[n_window_features*embed_size, hidden_size], dtype = tf.float32, initializer=tf.contrib.layers.xavier_initializer())
        b1 = tf.Variable(tf.zeros((hidden_size,)), name="b1")
        U = tf.get_variable("U", shape=[hidden_size, n_classes], dtype = tf.float32, initializer=tf.contrib.layers.xavier_initializer())
