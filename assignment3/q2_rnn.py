...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        padding   = max(0, max_length-len(sentence))
        unpadded  = min(len(sentence),max_length)
        sentence = sentence[:max_length]+padding*[zero_vector]
        labels   = labels[:max_length]+padding*[zero_label]
        masking  = [True]*unpadded+[False]*padding
        ret.append([sentence]+[labels]+[masking])
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        b2 = tf.get_variable('b2',(self.config.n_classes), tf.float32, initializer=tf.zeros_initializer(), )
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        cross_entropy_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=preds, labels=self.labels_placeholder)
        loss = tf.reduce_mean(tf.boolean_mask(cross_entropy_loss, self.mask_placeholder))
        ### END YOUR CODE
        return loss
